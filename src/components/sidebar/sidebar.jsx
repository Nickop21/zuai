...
function Sidebar() {
  ...
  function SideOne() {
    return (
      <div
        className={`w-[120px] md:w-[68px] md:p-2 fixed h-full top-0 md:left-0 transition-all duration-200    ${
          sideOpen
            ? "right-0 z-20 pt-[60px] animate-fade-in-side"
            : "-right-48 "
        }`}
      >
        <div className="bg-[#F8FAFC] flex justify-between  items-center flex-col gap-4 sm:rounded-sm h-full">
              ...
              return (
                <div key={index} onClick={()=>setActive(index)}>
                  ...
                  <Sideicons
                    ...
                    alt={item}
                    style={`h-9 w-9   ${active!==index?"bg-white hover:bg-[#6947bfbd]":"bg-[#6947BF] fill-[#FFFFFF]"}`}
                    link="/"
                  />
                </div>
              );
